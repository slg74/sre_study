...
- **What AWS manages**: control plane (etcd, API server, controller-manager, scheduler) across 3 AZs, patched and upgraded by AWS. You manage: node groups, add-ons, RBAC, networking config.
- **Node options**: **Managed node groups** (AWS drains/replaces nodes during upgrades, auto-registers with cluster); **Self-managed** (full control, you handle drain logic); **Fargate profiles** (serverless pods — no DaemonSets, no `hostNetwork`, no `privileged`, no EBS volumes, no GPU — stateless workloads only).
- **VPC CNI (aws-node DaemonSet)**: assigns real VPC IPs to pods from ENIs attached to the node. **Pod limit per node = (# ENIs × (IPs per ENI − 1)) + 2**. Nitro instance types support **prefix delegation** (`ENABLE_PREFIX_DELEGATION=true`) — assigns /28 prefixes instead of individual IPs, dramatically raising the pod ceiling. Choosing an ENI-limited instance type (e.g., `t3.small`: 3 ENIs × 3 IPs = 8 pods) is the #1 scheduling failure cause in new clusters.
- **IRSA (IAM Roles for Service Accounts)**: (1) Create OIDC provider for the cluster in IAM. (2) Annotate K8s SA: `eks.amazonaws.com/role-arn: arn:aws:iam::ACCOUNT:role/ROLE`. (3) IAM trust policy: Principal = `oidc.eks.REGION.amazonaws.com/id/OIDC_ID`, Condition `StringEquals` on `sub: system:serviceaccount:NAMESPACE:SA_NAME`. Missing the `sub` condition = any pod in the cluster can assume the role.
- **aws-auth ConfigMap** (in `kube-system`): maps IAM users/roles/node groups to K8s RBAC groups. Node groups auto-add `system:bootstrappers` + `system:nodes`. Human access: add `mapRoles` or `mapUsers` entries. Deleting or corrupting this ConfigMap locks everyone except the cluster creator out — keep a backup. EKS Access Entries (newer API) replaces ConfigMap for access management.
- **EKS add-ons** (managed upgrades via AWS): `vpc-cni`, `coredns`, `kube-proxy`, `aws-ebs-csi-driver`, `aws-efs-csi-driver`. Upgrade order matters: vpc-cni first (networking layer), then coredns + kube-proxy. Add-ons can conflict with self-managed versions — check for duplicates.
- **Cluster Autoscaler (CA) vs Karpenter**: CA watches unschedulable pods → triggers ASG scale-out (slow: ASG decides instance type, 3–5 min). **Karpenter** (newer): provisions nodes directly using EC2 Fleet, picks the optimal instance type from a NodePool spec, sub-60s from pending pod to running node. Karpenter also consolidates underutilized nodes automatically. CA requires one ASG per instance type/AZ; Karpenter is AZ-flexible.
- **Managed node group upgrade**: AWS cordon → drain → terminate → replace (one node at a time, or with `maxUnavailable`). Enable `updateConfig.maxUnavailable` for faster upgrades at the cost of temporary reduced capacity. PDBs are respected during drain — a misconfigured PDB blocks the upgrade indefinitely.
- **AWS Load Balancer Controller** (replaces in-tree LB controller): `Ingress` resource → provisions ALB (L7, path-based routing, target groups = pods). `Service` type `LoadBalancer` → provisions NLB (L4). Key annotations: `kubernetes.io/ingress.class: alb`, `alb.ingress.kubernetes.io/scheme: internet-facing|internal`, `alb.ingress.kubernetes.io/target-type: ip` (routes to pod IPs directly — requires VPC CNI, not instance).
- **Control plane logging** (opt-in to CloudWatch): `api` (every API call, high volume), `audit` (who did what — required for compliance), `authenticator` (IAM→RBAC mapping failures), `controllerManager`, `scheduler`. Enable `audit` at minimum. Costs money — filter to avoid noise.
- **Security posture**: Pod Security Standards (replace deprecated PodSecurityPolicy — PSP was removed in 1.25). Enforce via `PodSecurity` admission controller (namespace label `pod-security.kubernetes.io/enforce: restricted`). Secrets encryption: enable envelope encryption with a KMS key at cluster creation — encrypts etcd secrets at rest. Rotate the KMS key → re-encrypt all secrets.
- **Cluster upgrade strategy**: upgrade control plane first (AWS handles this), then node groups (old nodes can run on a newer control plane ±1 minor version). Never skip minor versions. Test add-ons compatibility before upgrading. EKS EOL = ~14 months per minor version — track with AWS deprecation notices.
